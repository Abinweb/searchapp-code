...
function renderResults(results, title, displayMode, maxItems, gridColumns = 3, paginationType = "None", container, currentPage = 1, isPageResult = true, styles = {}) {
    ...
    currentPage = Math.max(1, Math.min(currentPage, totalPages)); // Clamp currentPage between 1 and totalPages
    ...
    // Remove any existing pagination to prevent duplication
    if (container) {
...
">
          ...
          <!-- Single pagination section to prevent duplication -->
          ${paginationHtml}
      </section>`;

  if (container) {
      // Clear any existing pagination first to prevent duplication
      const existingPagination = container.querySelector('.pagination');
      if (existingPagination) {
          console.log('Removing existing pagination to prevent duplication');
          existingPagination.remove();
      }
      
      container.innerHTML = sectionHtml;
      
      // Debug: Log pagination creation
      if (paginationType !== "None") {
          console.log(`Created pagination: ${paginationType}, Current page: ${currentPage}, Total pages: ${totalPages}`);
      }
      
      // Add event listeners only to the pagination that was just created
